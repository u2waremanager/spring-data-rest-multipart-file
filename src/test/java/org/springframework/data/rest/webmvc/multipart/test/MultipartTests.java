...
public class MultipartTests extends ApplicationMockTests {

	@Value("${spring.data.rest.multipart.filesystem.location:target/storage}") 
	...
	public void contextLoads() throws Exception {
		...
		Files.list(Paths.get(location)).forEach(castConsumerWithThrowable((path)->{ 
			FileSystemUtils.deleteRecursively(path); 
		}));
		logger.info(fileSystemProperties.getLocation());
		...
		$.MULTIPART("/multipart").P("directory", "NewDirectory").is2xx("m1").and(docs.uploadDirectory("upload-step2"));
		$.MULTIPART("m1").P("directory", "NewDirectory").is2xx("m2").and(docs.uploadDirectory("upload-step3"));
		...
		// //create
		$.MULTIPART("/multipart").F(f1).is2xx("m3").and(docs.uploadFile("upload-step1"));
		...
		$.PUT("m5").C("Modify Contents..........").is2xx().and(docs.updateModify("update-step2"));
		$.GET("m5").is2xx();
